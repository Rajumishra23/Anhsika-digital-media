...
const Header = ({ modalActive }) => {
  ...
  return (
    <header className="bg-[#652f8e] shadow-md fixed top-0 left-0 right-0 z-50 transition-all">
      <div
        className={`max-w-7xl mx-auto px-4 md:px-6 flex items-center h-20 md:h-24 ${
          modalActive ? "justify-center" : "justify-between"
        }`}
      >
        {/* Logo (always visible) */}
        <Link to="/">
        ...
        {/* Full header content only when modal is not active */}
        {!modalActive && (
          <>
            {/* Mobile Menu Icon */}
